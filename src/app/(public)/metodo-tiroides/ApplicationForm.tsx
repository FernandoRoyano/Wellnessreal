    ...
    return (
      ...
      >
        ...
        <p className="mx-auto mt-3 max-w-md text-fluid-sm leading-relaxed text-muted">
          La revisaré personalmente y te escribiré para entender tu caso. Solicitar plaza no implica
          pagar ni reservar nada todavía.
        </p>
      </div>
    )
  }

  const fieldError = (name: keyof NonNullable<ThyroidApplicationState['fieldErrors']>) =>
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Field label="Nombre" error={fieldError('name')} errorId="application-name-error">
          <input
            name="name"
            autoComplete="name"
            required
            aria-invalid={Boolean(fieldError('name'))}
            aria-describedby={fieldError('name') ? 'application-name-error' : undefined}
            className={FIELD_CLASS}
            placeholder="Tu nombre"
          />
        </Field>
        <Field label="Email" error={fieldError('email')} errorId="application-email-error">
          <input
            name="email"
            type="email"
            inputMode="email"
            ...
            aria-describedby={fieldError('email') ? 'application-email-error' : undefined}
            className={FIELD_CLASS}
      ...
      <Field
        ...
        errorId="application-goal-error"
      >
        <textarea
          name="goal"
          rows={4}
          ...
          aria-describedby={fieldError('goal') ? 'application-goal-error' : undefined}
      ...
      <Field
        ...
        errorId="application-limitations-error"
      >
        <textarea
          name="limitations"
          rows={3}
          aria-invalid={Boolean(fieldError('limitations'))}
          aria-describedby={fieldError('limitations') ? 'application-limitations-error' : undefined}
          className={`${FIELD_CLASS} resize-y`}
          placeholder="No hace falta compartir analíticas ni medicación."
        />
      </Field>
      ...
      >
        {pending ? (
          ...
          <>
            Solicitar mi plaza <ArrowRight className="h-4 w-4" aria-hidden="true" />
          </>
        )}
      </button>
...
function Field({
  ...
  return (
    <label className="block">
      <span className="mb-1.5 block text-fluid-xs font-semibold text-white/80">{label}</span>
      {children}
      {error && (
        <span id={errorId} className="mt-1 block text-fluid-xs text-danger">
          {error}
        </span>
      )}
    </label>
