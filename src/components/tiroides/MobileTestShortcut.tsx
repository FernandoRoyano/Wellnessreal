...
export default function MobileTestShortcut() {
  ...
  useEffect(() => {
    ...
    if (!test) return () => window.removeEventListener('wr-consent-changed', handleConsent)
    const observer = new IntersectionObserver(
      ([entry]) => setIsTestVisible(entry.isIntersecting),
      { threshold: 0.18 },
    )
  ...
  if (isTestVisible || !hasConsentChoice) return null
...
export function MobileTestLink() {
  return (
    <a
      href="#test"
      onClick={() => trackThyroidFunnel('thyroid_landing_view', { view_type: 'cta_click', cta_position: 'hero' })}
      className="btn-brand w-full justify-center py-4 text-fluid-base lg:hidden"
    >
      Empezar el test · 1 minuto <ArrowRight className="h-4 w-4" />
    </a>
  )
